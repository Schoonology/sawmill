...
function format(body) {
  ...
  keys = keys.filter(function (key) {
    return !(key === 'level' || key === 'time' || key === 'mill' || key === 'message')
  })

  if (keys.length > 0) {
    body = keys.reduce(function (obj, key) {
      obj[key] = body[key]
      return obj
    }, {})
    ...
    message += util.inspect(body, { depth: null, colors: true })
  }

  return util.format('%s %s| %s: %s\n', mill.grey, time.grey, level, message)
}
